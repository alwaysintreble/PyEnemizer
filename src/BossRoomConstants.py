from enum import IntEnum


class DungeonID(IntEnum):
    tower_of_hera = 0
    swamp_palace = 7
    ice_palace = 8
    turtle_rock = 9
    ganons_tower_3 = 12


class BossRoomShell:
    room_shell_index: int
    room_shell: bytearray


class EasternRoomShell(BossRoomShell):
    room_shell_index = 13
    room_shell = {
        # type 1
        0xE1, 0x00,
        0x98, 0x92, 0x3A,
        0x88, 0xAA, 0x65,
        0xE8, 0xAA, 0x66,
        0xFF, 0xFF,
        0xAD, 0xA1, 0xF9,
        0xFF, 0xFF,
        # type 2
        0xF0, 0xFF,
        0x81, 0x18,
        0xFF, 0xFF,
    }


class DesertRoomShell(BossRoomShell):
    room_shell_index = 4
    room_shell = {
        # type 1
        0xE9, 0x00,
        0xFF, 0xFF,
        0x2D, 0xA1, 0xF9,
        0xFF, 0xFF,
        # type 2
        0xF0, 0xFF,
        0x61, 0x18,
        0xFF, 0xFF
    }


class ToHRoomShell(BossRoomShell):
    room_shell_index = 307
    room_shell = {
        # header
        0x81, 0x1C,
        # bg 1
        ,
        # bg 2
        0x61, 0x51, 0xF9,
        0xFF, 0xFF,
        # type 2
        0xF0, 0xFF,
        0xFF, 0xFF
    }


class PoDRoomShell(BossRoomShell):
    room_shell_index = 22
    room_shell = {
        # type 1
        0xE9, 0x00,
        0xA8, 0xA8, 0xDE,   0xB0, 0xA0, 0xDE,   0xB8, 0xA8, 0xDE,   0xC0, 0xA0, 0xDE,   0xC8, 0xA8, 0xDE,
        0xD0, 0xA0, 0xDE,
        0xFF, 0xFF,
        0xAD, 0xA1, 0xF9,
        0xFF, 0xFF,
        # type 2
        0xF0, 0xFF,
        0x81, 0x18,
        0xFF, 0xFF
    }


class SwampRoomShell(BossRoomShell):
    room_shell_index = 10
    room_shell = {
        # header
        0xE1, 0x00,
        # bg 1
        0x0C, 0xA5, 0x7F,
        0x6C, 0xA5, 0x80,
        0xFF, 0xFF,
        # bg 2
        0x2D, 0xA1, 0xF9,
        0xFF, 0xFF,
        # type 2
        0xF0, 0xFF,
        0x61, 0x18,
        0xFF, 0xFF
    }


class SkullRoomShell(BossRoomShell):
    room_shell_index = 0
    room_shell = {
        # type 1
        0xE5, 0x00,
        0x97, 0x9C, 0xDE,   0xB7, 0x9C, 0xDE,   0xD6, 0x9C, 0xDE,   0x97, 0xE4, 0xDE,   0xB7, 0xE4, 0xDE,
        0xD6, 0xE4, 0xDE,   0x94, 0xA7, 0xDE,   0x94, 0xC7, 0xDE,   0xE4, 0xA7, 0xDE,   0xE4, 0xC7, 0xDE,
        0xFF, 0xFF,
        0x03, 0x03, 0xCA,   0x43, 0x03, 0xCA,   0x83, 0x03, 0xCA,   0xC3, 0x03, 0xCA,   0x03, 0x43, 0xCA,
        0x43, 0x43, 0xCA,   0x83, 0x43, 0xCA,   0xC3, 0x43, 0xCA,   0x03, 0x83, 0xCA,   0x43, 0x83, 0xCA,
        0x83, 0x83, 0xCA,   0xC3, 0x83, 0xCA,   0x03, 0xC3, 0xCA,   0x43, 0xC3, 0xCA,   0x83, 0xC3, 0xCA,
        0xC3, 0xC3, 0xCA,
        0xFF, 0xFF,
        0x9F, 0xA7, 0xC6,   0xD4, 0xA7, 0xC6,   0xFE, 0xF9, 0xF4,   0xFF, 0x1E, 0x74,   0xFE, 0x5C, 0x74,
        0xFF, 0x9C, 0x74,
        # type 2
        0xF0, 0xFF,
        0xFF, 0xFF
    }


class ThievesRoomShell(BossRoomShell):
    room_shell_index = 34
    room_shell = {
        0xE9, 0x00,
        0x88, 0xA4, 0x0D,   0x88, 0xD0, 0x0E,   0xE0, 0x90, 0x0F,   0xE0, 0xE4, 0x10,   0x89, 0xAB, 0x61,
        0xE9, 0xAB, 0x62,   0x88, 0x91, 0xA0,   0x88, 0xE5, 0xA1,   0xE4, 0x91, 0xA2,   0xE4, 0xF5, 0xA3,
        0xFF, 0xFF,
        0xAD, 0xA1, 0xF9,   0xB1, 0xA8, 0xFF,
        0xFF, 0xFF,
        0xF0, 0xFF,
        0x81, 0x18,
        0xFF, 0xFF,
    }


class IceRoomShell(BossRoomShell):
    room_shell_index = 4
    room_shell = {
        # type 1
        0xE4, 0x00,
        0xFF, 0xFF,
        0xAD, 0x21, 0xF9,
        0xFF, 0xFF,
        # type 2
        0xF0, 0xFF,
        0xFF, 0xFF,
    }


class MireRoomShell(BossRoomShell):
    room_shell_index = 16
    room_shell = {
        # type 1
        0xE1, 0x00,
        0x28, 0xEC, 0x56,   0x48, 0xEC, 0x56,   0x1B, 0xA2, 0xFF,
        0xFF, 0xFF,
        0x16, 0x9C, 0xFE,   0x2D, 0xA1, 0xF9,
        0xFF, 0xFF,
        # type 2
        0xF0, 0xFF,
        0x61, 0x18,
        0xFF, 0xFF,
    }


class TurtleRoomShell(BossRoomShell):
    room_shell_index = 58
    room_shell = {
        # 0x2E, 0x98, 0xFF
        # type 2
        0xE1, 0x00,
        0xFC, 0x08, 0x00,   0x13, 0x80, 0x01,   0xFD, 0xC8, 0x02,   0x02, 0x93, 0x61,
        0xFC, 0x0E, 0x81,   0x13, 0xE8, 0x02,   0xFD, 0xCE, 0x83,   0x72, 0x93, 0x62,
        0x13, 0x93, 0xC4,   0x51, 0x93, 0xC4,   0x51, 0xC9, 0xC4,   0x10, 0xC9, 0xC4,
        0x0E, 0x8D, 0xDE,   0x0D, 0x9C, 0xDE,   0x0C, 0xA5, 0xDE,   0x5E, 0x8C, 0xDE,
        0x65, 0x94, 0xDE,   0x6C, 0x9C, 0xDE,
        0xFF, 0xFF,
        0x2D, 0xA1, 0xF9,
        # 2E 98 FF
        0xFF, 0xFF,
        # type 2
        0xF0, 0xFF,
        0x61, 0x18,
        0xFF, 0xFF
    }


class GT1RoomShell(BossRoomShell):
    room_shell_index = 55
    room_shell = {
        # type 1
        0xE1, 0x00,
        0x2D, 0x32, 0xA4,   0xA9, 0x1E, 0xDC,   0xA8, 0x91, 0x3A,   0x88, 0xAD, 0x76,   0xEC, 0xAD, 0x77,
        0xA8, 0x50, 0x3D,   0xD0, 0x50, 0x3D,   0x30, 0xA9, 0x3D,   0x30, 0xC1, 0x3D,   0xFC, 0x69, 0x38,
        0x97, 0x9F, 0xD1,   0xCD, 0x9F, 0xD1,   0x97, 0xDC, 0xD1,   0xCD, 0xDC, 0xD1,   0xBD, 0x32, 0xF9,
        0xB1, 0x22, 0xF9,   0xC9, 0x22, 0xF9,
        0xFF, 0xFF,
        0xAD, 0xA1, 0xF9,
        0xFF, 0xFF,
        # type 2
        0xF0, 0xFF,
        0x80, 0x36, 0x82,   0x18, 0x60, 0x28,
        0xFF, 0xFF,
    }


class GT2RoomShell(BossRoomShell):
    room_shell_index = 70
    room_shell = {
        # type 1
        0xE2, 0x00,
        0x17, 0x9F, 0xE8,   0x4D, 0x9F, 0xE8,   0x17, 0xDC, 0xE8,   0x4D, 0xDC, 0xE8,   0x18, 0xE1, 0xFE,
        0x88, 0xAD, 0x76,   0x99, 0xBC, 0x33,   0x9B, 0xBB, 0x34,   0x9B, 0xCF, 0x34,   0xD8, 0xB8, 0x34,
        0xD8, 0xCC, 0x34,   0xAF, 0xAA, 0xFE,   0xC7, 0xAA, 0xFE,   0xAF, 0xD2, 0xFE,   0xC7, 0xD2, 0xFE,
        0x28, 0x11, 0x3A,   0x28, 0x91, 0x3A,   0xFC, 0xE1, 0x38,   0x2B, 0x33, 0xFA,   0x53, 0x33, 0xFA,
        0x2B, 0x53, 0xFA,   0x53, 0x53, 0xFA,
        0xFF, 0xFF,
        0x2D, 0xA1, 0xF9,
        0xFF, 0xFF,
        # type 2
        0xF0, 0xFF,
        0x82, 0x38, 0x60,   0x18, 0x83, 0x00,
        0xFF, 0xFF,
    }


class GT3RoomShell(BossRoomShell):
    room_shell_index = 244
    room_shell = {
        # type 1
        ,
        #                                 validation chest                                        #
        ,
        # 69 61 F9  # kholdstare
        # type 2
        0xF0, 0xFF,
        0x00, 0x1C, 0x22,   0x00,
        0xFF, 0xFF,
    }
